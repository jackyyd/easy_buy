...
from apps.goods.models import SKUSpecification, SKU, GoodsCategory, SPU, SpecificationOption, SPUSpecification
from django.db import transaction
from celery_tasks.sku_html.tasks import generate_static_sku_detail_html


class SKUSpecOptModelSerializer(serializers.ModelSerializer):

    # 因为这两个字段必须参与反序列化,所有我们只能手动映射覆盖原有的默认映射(默认只作用于序列化)
    spec_id = serializers.IntegerField()
    option_id = serializers.IntegerField()

    class Meta:
        model = SKUSpecification
        # 如果关联对象主键隐藏字段，我们在fields中显示声明,
        # 则会自动映射类型为ReadOnlyField--只读(只参与序列化)
        fields = [
            'spec_id',
            'option_id'
        ]
...
class SKUModelSerializer(serializers.ModelSerializer):
    # 三个隐藏字段
    # 1. 本表主键id,会自动映射
    # 2. 外键关联对象的主键隐藏字段(spu_id),不会自动映射
    spu = serializers.StringRelatedField()
    spu_id = serializers.IntegerField()
    category = serializers.StringRelatedField()
    category_id = serializers.IntegerField()

    # 3.从表related_name指定的是主表隐藏字段,不会自动映射
    ...
    def create(self, validated_data):
        ...
        # TODO:　下面两部操作，是数据库两张表的插入动作，必须保证"事务"特性
    ...
    def update(self, instance, validated_data):
        # [{spec_id: "1", option_id: 1}, {spec_id: "2", option_id: 4}, {specid: "3", option_id: 6}
        specs = validated_data.pop('specs')
...
class SKUCategorySimpleSerialzier(serializers.ModelSerializer):
    class Meta:
        model = GoodsCategory
        fields = [
            'id',
